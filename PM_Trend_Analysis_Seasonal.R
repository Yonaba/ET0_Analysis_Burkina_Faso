...
season <- "wet"
dry_months <- c(1,2,3,4,5,11,12)
wet_months <- c(6,7,8,9,10)
...
for (s in stations) {
  ...
  data.d$m <- as.numeric(format(data.d$dates,"%m"))
  
  if (season == "dry") cmonths <- dry_months else c_months <- wet_months
  data.d <- subset(data.d, (data.d$m %in% c_months))
  data.d$m <- NULL
